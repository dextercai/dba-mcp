...
- 所有 Oracle 资产均可执行非变更 MCP 工具；`read_only=true` 只禁止变更工具。`oracle.unlockUser` 是受控例外而非通用 DDL，必须使用 `read_only=false` 的专用资产。它默认关闭，只有运行时 `DBA_ORACLE_USER_UNLOCK_ENABLED=true` 和目标资产 `user_unlock_enabled=1` 同时成立才可调用。它仅接受未加引号 Oracle 用户标识符；固定预检角色继承、系统权限和 password-file 管理权限，任何 DBA 类角色、广泛 `ANY` 权限、`ALTER USER` 等管理权限、password-file 管理权限或预检错误都会拒绝操作。
- Oracle 连接端点和驱动属性可保存在资产 SQLite。可选 `connection_properties.jdbcUrl` 只能保存不含凭据的 Oracle Thin 端点；认证统一使用独立的 `username` / `password`。资产管理 REST API 可在通过 HTTPS 的 Basic Auth 保护下写入数据库密码（`connection_properties.password`）；该字段为严格只写，省略或传递 `null` 不会清除已有密码，所有 REST/MCP 响应、日志、审计参数和异常消息均不得包含它。令牌、SSH 私钥和钱包密钥仍不得存入 SQLite。
