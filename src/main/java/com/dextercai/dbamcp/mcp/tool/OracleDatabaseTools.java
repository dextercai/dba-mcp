...
public class OracleDatabaseTools {
    ...
    @Tool(name = "oracle.listConstraints", description = "List constraints across one Oracle schema on a registered read-only asset. Use describeTable for one table; owner must be an unquoted Oracle identifier; results are bounded.")
    public QueryResult listConstraints(String assetId, String owner) { return databases.listConstraints(assetId, owner); }
    @Tool(name = "oracle.listIndexes", description = "List indexes across one Oracle schema on a registered read-only asset. Use describeTable for one table; owner must be an unquoted Oracle identifier; results are bounded.")
    public QueryResult listIndexes(String assetId, String owner) { return databases.listIndexes(assetId, owner); }
    @Tool(name = "oracle.listAlertLogEvents", description = "List one offset-based page of recent Oracle ADR alert events for the current container on a registered read-only asset. offset defaults to 0; pageSize defaults to 100 and is 1-500. Raw message text is returned only when the server-side sensitive-message setting is explicitly enabled.")
